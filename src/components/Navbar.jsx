    ...
    className={`${styles.paddingX} w-full flex item-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" 
            ...
            <p className='text-white text-[18px] font-bold cursor-pointer flex'>Abdurrahman &nbsp;<span className='sm:block hidden '>CS/DT Analyst</span> </p>
        </Link>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          <div className={`${!toggle ? 'hidden' : 'flex'} p-6 black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
            ...
            {navLinks.map((link) => (
            <li key={Link.id} className={`${active === Link.title ? "text-white" : "text-secondary"} font-poppins font-medium cursor-pointer text-[16px]`} onClick={()=>{setActive(Link.title); setToggle(false)}}>
              <a href={`#${link.id}`}>{link.title}</a>
            </li>
            ))}
        </ul>
